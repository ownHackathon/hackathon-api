...
test('workspace input validators cover optional and bounded fields', function (): void {
    ...
    $visibility->setValue(' 700 ');
    expect($visibility->isValid())->toBeTrue()->and($visibility->getValue())->toBe('700');
...
test('composed workspace and email validators validate complete payloads', function (): void {
    ...
    $workspace->setData(['name' => 'Team', 'description' => '', 'details' => '', 'visibility' => 700]);
    expect($workspace->isValid())->toBeTrue();
    $workspace->setData(['name' => 'x', 'description' => str_repeat('x', 256), 'details' => '', 'visibility' => 701]);
