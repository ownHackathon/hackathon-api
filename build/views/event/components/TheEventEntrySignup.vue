<template>
  <div v-show="userService.isAuthenticated() && eventService.canStillParticipate(data.status)">
    <div v-if="!eventService.isUserInParticipantList(data.participants, userStore.user)" class="flex">
      <button class="button" @click="eventService.addUserAsParticipantToEvent(userStore.user.uuid, route.params.id)">Zum Event <span class="text-green-600">anmelden</span></button>
    </div>
    <div v-else>
