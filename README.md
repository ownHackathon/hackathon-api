...
| `testing` | `phpunit.xml` / `tests/Pest.php` | Test suite – used for every test run (local and CI) |
| `action` | GitHub Actions workflows | CI/build overrides (e.g. migrations during build jobs) |
...
| `*.testing.php` | `APP_ENV=testing` | Test settings for every test run (local and CI). E.g. `database.testing.php` uses the `database-testing` container by default or `DB_HOST`/`DB_PORT` when set (CI), `token.testing.php` longer refresh durations |
| `*.action.php` | `APP_ENV=action` | CI/build settings. E.g. `database.action.php` connects to `127.0.0.1`, `dependencies.action.php` injects `NullMailer`/`NullLogger` mocks |
...
- The merged configuration is cached per environment to
  `data/cache/<APP_ENV>-config-cache.php`, so environments never overwrite each
  other's cache. The cache is disabled in development; enable it via
  `config/autoload/local.php` and clear it after every deployment with
